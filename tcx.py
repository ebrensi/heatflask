...
#
# Efrem note: got this from https://github.com/jhofman/fitnesshacks
...
def parsetcx(xml):
    ...
    for lap in tcx.findall('.//Lap/'):
        for point in lap.findall('.//Trackpoint'):
            ...
            if timestamp:
                seconds = strftime(
                    '%s', strptime(timestamp, "%Y-%m-%dT%H:%M:%S.%fZ"))
            else:
                seconds = None
            ...
            xy_pos = (findtext(position, 'LatitudeDegrees'),
                      findtext(position, 'LongitudeDegrees'))
