...
export default new Namer({
  name({ bundle, bundleGraph }) {
    ...
    const bundleGroupBundles = bundleGraph.getBundlesInBundleGroup(
      bundleGroup,
      {
        includeInline: true,
      }
    ...
    if (
      ...
      const loc = bundle.target.loc
      const distEntry = bundle.target.distEntry
      const distExtension = path.extname(bundle.target.distEntry).slice(1)
      const allowedExtensions = ALLOWED_EXTENSIONS[bundle.type] || [bundle.type]
