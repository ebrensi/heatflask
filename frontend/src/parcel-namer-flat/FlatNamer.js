...
var _default = new (_plugin().Namer)({
  name({ bundle, bundleGraph }) {
    ...
    if (bundle.needsStableName) {
      ...
      ;(0, _assert().default)(
        entryBundlesOfType.length === 1, // Otherwise, we'd end up naming two bundles the same thing.
    ...
    if (
      ...
      let distExtension = _path()
        .default.extname(bundle.target.distEntry)
        .slice(1)
      ...
      if (!allowedExtensions.includes(distExtension) && loc) {
        ...
        let fullName = _path().default.relative(
          // _path().default.dirname(loc.filePath),
          _path().default.join(bundle.target.distDir, distEntry)
        )
        ...
        let err = new (_diagnostic().default)({
          diagnostic: {
            message: (0,
            _diagnostic()
              .md)`Target "${bundle.target.name}" declares an output file path of "${fullName}" which does not match the compiled bundle type "${bundle.type}".`,
            codeFrames: [
              {
                ...
                codeHighlights: [
                  {
                    ...
                    message: (0, _diagnostic().md)`Did you mean "${
                      fullName.slice(
                        0,
                        -_path().default.extname(fullName).length
                      ) +
                      "." +
                      bundle.type
                    }"?`,
            ...
            hints: [
              `Try changing the file extension of "${
                bundle.target.name
              }" in ${_path().default.relative(process.cwd(), loc.filePath)}.`,
            ],
          },
        })
        throw err
...
function nameFromContent(bundle, isEntry, entryAssetId, entryRoot) {
  let entryFilePath = (0, _nullthrows().default)(
