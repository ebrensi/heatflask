...
import * as CRS from "../DotLayer/CRS.js"
...
export const ViewBox = {
  initialize: function (map, canvases, itemsArray) {
    ...
    if (itemsArray) this.setItemsArray(itemsArray)
  ...
  updateView: function () {
    ...
    for (let i = 0, len = allItems.length; i < len; i++) {
      if (this.overlaps(allItems[i].pxBounds)) currentInView.add(i)
    }
  ...
  update: function (calibrate = true) {
    ...
    if (zoomChange) this.onZoomChange(zoom)
