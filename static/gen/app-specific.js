...
return outArray;}
const SPEED_SCALE=5.0,SEP_SCALE={m:0.14,b:15.0};var map_providers=ONLOAD_PARAMS.map_providers,baseLayers={"None":L.tileLayer("")},default_baseLayer=baseLayers["None"],HeatLayer=false,FlowLayer=false,DotLayer=false,appState={baseLayers:map_providers,paused:ONLOAD_PARAMS.start_paused,items:{}};if(!OFFLINE){var online_baseLayers={"Esri.WorldImagery":L.tileLayer.provider("Esri.WorldImagery"),"Stamen.Terrain":L.tileLayer.provider("Stamen.Terrain"),"OpenStreetMap.BlackAndWhite":L.tileLayer.provider("OpenStreetMap.BlackAndWhite"),"CartoDB.Positron":L.tileLayer.provider("CartoDB.Positron"),"CartoDB.DarkMatter":L.tileLayer.provider("CartoDB.DarkMatter"),"Google.Roadmap":L.gridLayer.googleMutant({type:'roadmap'}),"Google.Terrain":L.gridLayer.googleMutant({type:'terrain'})};Object.assign(baseLayers,online_baseLayers);if(map_providers.length){for(var i=0;i<map_providers.length;i++){provider=map_providers[i];var tl=L.tileLayer.provider(provider);baseLayers[provider]=tl;if(i==0)default_baseLayer=tl;}}else{default_baseLayer=baseLayers["Google.Terrain"];}}
var map=L.map('map',{center:ONLOAD_PARAMS.map_center,zoom:ONLOAD_PARAMS.map_zoom,layers:[default_baseLayer],preferCanvas:true,zoomAnimation:false});var sidebarControl=L.control.sidebar('sidebar').addTo(map),zoomControl=map.zoomControl.setPosition('bottomright'),layerControl=L.control.layers(baseLayers,null,{position:'topleft'}).addTo(map),fps_display=ADMIN?L.control.fps().addTo(map):null;var animation_button_states=[{stateName:'animation-running',icon:'fa-pause',title:'Pause Animation',onClick:function(btn,map){pauseFlow();updateState();btn.state('animation-paused');}},{stateName:'animation-paused',icon:'fa-play',title:'Resume Animation',onClick:function(btn,map){resumeFlow();if(DotLayer){DotLayer.animate();}
updateState();btn.state('animation-running');}}];var animationControl=L.easyButton({states:appState.paused?animation_button_states.reverse():animation_button_states}).addTo(map);var capture_button_states=[{stateName:'not-capturing',icon:'fa-video-camera',title:'Capture',onClick:function(btn,map){if(!DotLayer){return;}
btn.state('capturing');timeout=DotLayer.captureCycle();setTimeout(function(){btn.state('not-capturing');},timeout+500);}},{stateName:'capturing',icon:'fa-stop',title:'Stop capturing',onClick:function(btn,map){if(DotLayer&&DotLayer._capturing){DotLayer.abortCapture();}}}];var captureControl=L.easyButton({states:capture_button_states});captureControl.enabled=false;$(".dotconst-dial").knob({min:0,max:100,step:0.1,width:"150",height:"150",cursor:20,inline:true,displayInput:false,change:function(val){if(!DotLayer){return;}
...
function highlightPath(id){var A=appState.items[id];if(A.selected)return false;A.highlighted=true;var row=$("#"+id),scroller=$('.dataTables_scrollBody'),flow=A.flowLayer;row.addClass('selected');scroller.scrollTop(row.prop('offsetTop')-scroller.height()/2);return A;}
...
function renderLayers(){const flowres=$("#flowres").val(),heatres=$("#heatres").val(),date1=$("#date1").val(),date2=$("#date2").val(),type=$("#select_type").val(),num=$("#select_num").val(),lores=flowres=="low"||heatres=="low",hires=flowres=="high"||heatres=="high";dotFlow=true;var query={};if(type=="activity_ids"){query.id=$("#activity_ids").val();}else if(type=="activities"){if(num==0){query.limit=1;}else{query.limit=num;}}else{if(date1){query.date1=date1;}
if(date2&&date2!="now"){query.date2=date2;}}
if(hires){query.hires=hires;}
if(HeatLayer){map.removeLayer(HeatLayer);layerControl.removeLayer(HeatLayer);HeatLayer=false;}
if(DotLayer){map.removeLayer(DotLayer);layerControl.removeLayer(DotLayer);DotLayer=false;}
var latlngs_flat=[];if(heatres){HeatLayer=L.heatLayer(latlngs_flat,HEATLAYER_DEFAULT_OPTIONS);map.addLayer(HeatLayer);layerControl.addOverlay(HeatLayer,"Point Density");}
var toDelete=new Set(Object.keys(appState.items));atable.clear();var msgBox=L.control.window(map,{position:'top',content:"<div class='data_message'></div><div><progress class='progbar' id='box'></progress></div>",visible:true}),progress_bars=$('.progbar'),rendering=true,listening=true,bounds=L.latLngBounds(),source=new EventSource(BASE_DATAURL+"?"+jQuery.param(query,true));$(".data_message").html("Rendering activities...");$("#abortButton").show();$(".progbar").show();$('#renderButton').prop('disabled',true);function doneRendering(msg){if(rendering){$("#abortButton").hide();$(".progbar").hide();try{msgBox.close();}catch(err){console.log(err.message);}
if($("#autozoom:checked").val()&&bounds.isValid())map.fitBounds(bounds);var msg2=msg+" "+Object.keys(appState.items).length+" activities rendered.";$(".data_message").html(msg2);rendering=false;if(dotFlow){DotLayer=new L.DotLayer(appState.items,{startPaused:appState.paused});map.addLayer(DotLayer);layerControl.addOverlay(DotLayer,"Dots");$("#sepConst").val((Math.log2(DotLayer.C1)-SEP_SCALE.b)/SEP_SCALE.m).trigger("change");$("#speedConst").val(Math.sqrt(DotLayer.C2)/SPEED_SCALE).trigger("change");setTimeout(function(){$("#period-value").html(DotLayer.periodInSecs().toFixed(2));},500);$("#showPaths").prop("checked",DotLayer.options.showPaths).on("change",function(){DotLayer.options.showPaths=$(this).prop("checked");DotLayer._onLayerDidMove();});for(let item of toDelete){delete appState.items[item];}
atable.rows.add(Object.values(appState.items)).draw(false);}}}
function stopListening(){if(listening){listening=false;source.close();$('#renderButton').prop('disabled',false);}}
source.onmessage=function(event){if(event.data=='done'){doneRendering("Finished. ");stopListening();appState['date1']=date1;appState["date2"]=date2;appState["flowres"]=flowres;appState["heatres"]=heatres;if("limit"in query)appState["limit"]=query.limit;updateState();}else{var A=JSON.parse(event.data),heatpoints=false,flowpoints=false;A.selected=false;A.bounds=L.latLngBounds();if("error"in A){var msg="<font color='red'>"+A.error+"</font><br>";$(".data_message").html(A.msg);console.log(`Error activity ${A.id}:${A.error}`);return;}else if("stop_rendering"in A){doneRendering("Done rendering.");}else if("msg"in A){$(".data_message").html(A.msg);if("value"in A){progress_bars.val(A.value);}
return;}else{let alreadyIn=toDelete.delete(A.id.toString());if(!heatres&&alreadyIn){return;}}
if(lores&&"summary_polyline"in A&&A.summary_polyline){let latlngs=L.PolylineUtil.decode(A.summary_polyline);if(heatres=="low")heatpoints=latlngs;}
if(query.hires&&"polyline"in A&&A.polyline){let latLngArray=L.PolylineUtil.decode(A.polyline);if(heatres=="high")heatpoints=latLngArray;if(flowres=="high"&&"time"in A){let len=latLngArray.length,time=streamDecode(A.time),latLngTime=new Float32Array(3*len);for(let i=0,ll;i<len;i++){ll=latLngArray[i];A.bounds.extend(ll);idx=i*3;latLngTime[idx]=ll[0];latLngTime[idx+1]=ll[1];latLngTime[idx+2]=time[i];}
A.latLngTime=latLngTime;flowpoints=latLngTime;}}
if(heatpoints){latlngs_flat.push.apply(latlngs_flat,heatpoints);}
if(heatpoints||flowpoints){A.startTime=moment(A.ts_UTC||A.ts_local).valueOf();bounds.extend(A.bounds);delete A.summary_polyline;delete A.polyline;delete A.time;if(!(A.id in appState.items)){appState.items[A.id]=A;}}}};}
...
const perf_t0=performance.now();this._zoom=this._map.getZoom();this._center=this._map.getCenter;this._size=this._map.getSize();this._latLngBounds=this._map.getBounds();this._mapPanePos=this._map._getMapPanePos();this._pxOrigin=this._map.getPixelOrigin();this._pxBounds=this._map.getPixelBounds();this._pxOffset=this._mapPanePos.subtract(this._pxOrigin)._add(new L.Point(0.5,0.5));var lineCtx=this._lineCtx,z=this._zoom,ppos=this._mapPanePos,pxOrigin=this._pxOrigin,pxBounds=this._pxBounds,items=this._items;this._dotCtx.strokeStyle=this.options.selected.dotStrokeColor;this._dotSize=Math.max(1,~~(Math.log(z)+0.5));this._dotOffset=~~(this._dotSize/2+0.5);this._zoomFactor=1/Math.pow(2,z);var tThresh=this._tThresh*DotLayer._zoomFactor;this._processedItems={};let pxOffx=this._pxOffset.x,pxOffy=this._pxOffset.y;for(let id in items){if(!items.hasOwnProperty(id)){continue;}
...
elapsed=(performance.now()-perf_t0).toFixed(2);},drawDots:function(obj,now,highlighted){var P=obj.P,dP=obj.dP,len_dP=dP.length,totSec=obj.totSec,period=this._period,s=this._timeScale*(now-obj.startTime),xmax=this._size.x,ymax=this._size.y,ctx=this._dotCtx,dotSize=this._dotSize,dotOffset=this._dotOffset,two_pi=this.two_pi,xOffset=this._pxOffset.x,yOffset=this._pxOffset.y;var timeOffset=s%period,count=0,idx=dP[0],dx=dP[1],dy=dP[2],p=P.slice(idx,idx+3);if(this.options.colorAll||highlighted){ctx.fillStyle=obj.dotColor||this.options.selected.dotColor;}
...
dt=t-p[2];if(dt>0){let lx=~~(p[0]+dx*dt+xOffset),ly=~~(p[1]+dy*dt+yOffset);if(lx>=0&&lx<=xmax&&ly>=0&&ly<=ymax){if(highlighted){ctx.beginPath();ctx.arc(lx,ly,dotSize,0,two_pi);ctx.fill();ctx.closePath();ctx.stroke();}else{ctx.fillRect(lx-dotOffset,ly-dotOffset,dotSize,dotSize);}
count++;}}}
...
this._frame=null;let ts=Date.now(),now=ts-this._timeOffset,capturing=this._capturing;if(this._paused||this._mapMoving){this._timePaused=ts;return;}
if(capturing||now-this.lastCalledTime>this.minDelay){this.lastCalledTime=now;this.drawLayer(now);capturing&&this._capturer.capture(this._dotCanvas);}
this._frame=L.Util.requestAnimFrame(this._animate,this);},_setTransform:function(el,offset,scale){var pos=offset||new L.Point(0,0);el.style[L.DomUtil.TRANSFORM]=(L.Browser.ie3d?"translate("+pos.x+"px,"+pos.y+"px)":"translate3d("+pos.x+"px,"+pos.y+"px,0)")+(scale?" scale("+scale+")":"");},_animateZoom:function(e){var scale=this._map.getZoomScale(e.zoom);var offset=L.Layer?this._map._latLngToNewLayerPoint(this._map.getBounds().getNorthWest(),e.zoom,e.center):this._map._getCenterOffset(e.center)._multiplyBy(-scale).subtract(this._map._getMapPanePos());L.DomUtil.setTransform(this._dotCanvas,offset,scale);L.DomUtil.setTransform(this._lineCanvas,offset,scale);},periodInSecs:function(){return this._period/(this._timeScale*1000);},captureCycle:function(){let periodInSecs=this.periodInSecs();this._mapMoving=true;pd=document.createElement('div');pd.style.position='absolute';pd.style.left=pd.style.top=0;pd.style.backgroundColor='black';pd.style.fontFamily='monospace';pd.style.fontSize='20px';pd.style.padding='5px';pd.style.color='white';pd.style.zIndex=100000;document.body.appendChild(pd);this._progressDisplay=pd;let msg="capturing static map frame...";pd.textContent=msg;leafletImage(this._map,function(err,canvas){console.log("leaflet-image: "+err);if(canvas){this.captureGIF(canvas,periodInSecs);}}.bind(this));},captureGIF:function(baseCanvas=null,durationSecs=2){this._mapMoving=true;let height=baseCanvas?baseCanvas.height:this._size.y,width=baseCanvas?baseCanvas.width:this._size.x,frameCanvas=document.createElement('canvas'),pd=this._progressDisplay;frameCanvas.width=width;frameCanvas.height=height;let frameCtx=frameCanvas.getContext('2d'),frameTime=Date.now(),frameRate=30,numFrames=durationSecs*frameRate,delay=1000/frameRate,encoder=new GIF({workers:4,quality:10,workerScript:'static/js/gif.worker.js'});this._encoder=encoder;encoder.on('progress',function(p){msg=`Encoding frames...${~~(p*100)}%`;this._progressDisplay.textContent=msg;}.bind(this));encoder.on('finished',function(blob){download(blob,"output.gif",'image/gif');document.body.removeChild(this._progressDisplay);delete this._progressDisplay;this._mapMoving=false;if(!this._paused){this.animate();}}.bind(this));for(let i=0,num=Math.round(numFrames);i<num;i++,frameTime+=delay){msg=`Rendering frames...${~~(i/num*100)}%`;pd.textContent=msg;this.drawLayer(frameTime);frameCtx.clearRect(0,0,width,height);frameCtx.drawImage(baseCanvas,0,0);frameCtx.drawImage(this._dotCanvas,0,0);encoder.addFrame(frameCanvas,{copy:true,delay:delay});}
encoder.render();},abortCapture:function(){console.log("abort request");this._progressDisplay.textContent("aborting...");if(this._encoder){this._encoder.abort();}}});L.dotLayer=function(items,options){return new L.DotLayer(items,options);};function makeColorGradient(frequency1,frequency2,frequency3,phase1,phase2,phase3,center,width,len){let palette=new Array(len);if(center==undefined)center=128;if(width==undefined)width=127;if(len==undefined)len=50;for(let i=0;i<len;++i){let r=Math.round(Math.sin(frequency1*i+phase1)*width+center),g=Math.round(Math.sin(frequency2*i+phase2)*width+center),b=Math.round(Math.sin(frequency3*i+phase3)*width+center);palette[i]=`rgb(${r},${g},${b})`;}
return palette;}
